...
## 1. Intro
Es ist sehr praktikum, wenn man Texte oder E-Mails auf korrekte Rechtschreibung überprüfen lassen kann, so dass ich mich im Fokus halte und nicht auf Groß- und Kleinbuchstaben achten muss.

Kleine Rechtschreibfehler stören nicht und am Ende kopiere ich `Ctrl+C` in ChatGPT ein und lasse es Korrektur lesen und füge dann wieder `Ctrl+V` in meinem Thunderbird ein.
